...
export default function RegisterPage() {
  ...
  // If user logs in (e.g. via auto-session after signup), go home
  useEffect(() => {
    if (user) navigate('/')
  }, [user, navigate])
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (result?.session) {
        // Email confirm disabled — user is logged in, useEffect handles redirect
        return
      }
      setSent(true)
    } catch (err) {
    ...
    <div className="min-h-screen bg-black flex items-center justify-center px-6 py-24">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-text-dim text-sm mt-1.5">Join the smart wearable platform</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-white text-black font-medium text-sm hover:bg-white/90 transition-all duration-300 disabled:opacity-50 mt-6"
          >
